...
function renderSajuResult(name, typeName, year, month, day, resultData, fortuneType, bazi, wuXing, isUnknownTime) {
    ...
    document.getElementById('resultTitle').innerHTML = "<span style='font-size: 0.65em; color: #FFD700;'>" + name + "님을 위한 명리 컨설팅</span><br><span style='font-size: 1.15em; display: inline-block; margin-top: 15px; color:#fff;'>" + typeName + "</span>";

    let chartHTML = (fortuneType === 'wealth') ? "" : generateSajuChartsHTML(colorInfo, bazi, wuXing, isUnknownTime);

    // 💡 박스 배경색을 아예 투명(transparent)으로 만들어서, 배경 사진이 글씨 뒤에 바로 보이게 합니다.
    ...
    let goldenScrollHTML = "<div class='free-preview' style='background: transparent; padding: 40px 10px; text-align: center; margin-bottom: 30px;'>" +
        ...
        "</div>" +
        "<p style='color:#fff; font-size: 1.1rem; line-height: 1.8; text-align: justify; word-break: keep-all; margin: 0; padding: 20px; background: rgba(0,0,0,0.3); border-radius: 10px;'>" + safeSummary + "</p>" +
        "</div>";
