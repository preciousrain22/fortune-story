...
window.loginWithKakao = function () {
    window.selectPath('gateway'); // 로그인 후 게이트웨이로 넘어가는 핵심 코드!
    try {
...
function renderSajuResult(name, typeName, year, month, day, resultData, fortuneType, bazi, wuXing, isUnknownTime) {
    ...
    document.getElementById('resultTitle').innerHTML = "<span style='font-size: 0.65em; color: #FFD700; text-shadow: 1px 1px 3px #000;'>" + name + "님을 위한 명리 컨설팅</span><br><span style='font-size: 1.15em; display: inline-block; margin-top: 15px; color:#fff; text-shadow: 1px 1px 3px #000;'>" + typeName + "</span>";
    ...
    let goldenScrollHTML = "<div class='free-preview' style='background: transparent; padding: 40px 10px; text-align: center; margin-bottom: 30px;'>" +
        "<div style='margin-bottom: 30px;'>" +
        "<div style='font-size: 1.6rem; font-weight: 900; color: #E5C07B; margin-bottom: 10px; text-shadow: 2px 2px 4px #000;'>" + (resultData.keyword1 || "거대한 조력자") + "</div>" +
        "<div style='font-size: 2.4rem; font-weight: 900; color: #FFD700; margin-bottom: 10px; text-shadow: 3px 3px 6px #000;'>" + (resultData.keyword2 || "제왕의 기틀") + "</div>" +
        "<div style='font-size: 1.6rem; font-weight: 900; color: #E5C07B; text-shadow: 2px 2px 4px #000;'>" + (resultData.keyword3 || "중차대한 전환점") + "</div>" +
        "</div>" +
        "<p style='color:#fff; font-size: 1.1rem; line-height: 1.8; text-align: justify; word-break: keep-all; margin: 0; padding: 20px; background: rgba(0,0,0,0.4); border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.5);'>" + safeSummary + "</p>" +
        "</div>";
    ...
    if (resultData.scores) {
        ...
        premiumHTML += "<div style='margin-top: 1rem; margin-bottom: 3rem; padding: 2rem; background: rgba(0,0,0,0.6); border-radius: 15px; border: 1px solid rgba(212, 175, 55, 0.3); box-shadow: 0 4px 15px rgba(0,0,0,0.5);'>" +
